...
function concatenateInLinuxOrMac(files) {
    var uploadsFolder = '/Users/songyanho/Developer/notbadsia/uploads/' + files.roomId + '/';
    ...
    var ffmpegCommand = ffmpeg().addOption('-ac 2').mergeAdd(allFiles[0]);
    allFiles.forEach(function(filePath, idx) {
        if (idx !== 0) {
            ffmpegCommand = ffmpegCommand.mergeAdd(filePath);
        }
    });
    ...
    ffmpegCommand.videoCodec('libx264').addOption('-strict', 'experimental').inputOptions(['-ac 2', '-c:a libvorbis']).mergeToFile(final_file, '/Users/songyanho/Developer/notbadsia/temp-uploads/');
}
